import { FileText, GitCommit, FolderTree, ArrowRight, User, Calendar, Hash, Link } from 'lucide-react';
...
export function ObjectDetail({ object, allObjects, onSelectObject }: ObjectDetailProps) {
  const getObjectByHash = (hash: string) => {
    return allObjects.find(o => o.hash === hash);
  };
  
  const renderCommitDetail = (commit: CommitObject) => {
    const treeObj = getObjectByHash(commit.tree);
      ...
      <div className="space-y-4">
        <div className="bg-blue-500/5 border border-blue-500/20 rounded p-4">
          ...
          <p className="text-xs text-gray-400 leading-relaxed">
            A commit is a snapshot of your project. It contains a pointer to a tree object (the file structure), 
            parent commit(s), author info, and a message.
          </p>
        </div>
        ...
        <div>
          ...
          <div className="bg-[#252526] rounded p-3">
            ...
            >
              ...
              <code className="text-xs text-green-400 font-mono flex-1 text-left">{commit.tree}</code>
            ...
            <div className="bg-[#252526] rounded p-3 space-y-2">
              ...
              {parentObjs.map((parent) => parent && (
                ...
                >
                  ...
                  <code className="text-xs text-blue-400 font-mono flex-1 text-left">{parent.hash}</code>
  ...
  const renderTreeDetail = (tree: TreeObject) => {
    return (
      <div className="space-y-4">
        <div className="bg-green-500/5 border border-green-500/20 rounded p-4">
          ...
          <p className="text-xs text-gray-400 leading-relaxed">
            A tree represents a directory. It contains pointers to blobs (files) and other trees (subdirectories), 
            along with their names and permissions.
          </p>
        </div>
                ...
                >
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <code className="text-xs text-gray-500">{entry.hash.substring(0, 12)}...</code>
                      {entryObj && <span className="text-xs text-gray-500">{entryObj.size} bytes</span>}
                    </div>
                  </div>
        ...
        {tree.referencedBy && tree.referencedBy.length > 0 && (
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs text-gray-400">Referenced By ({tree.referencedBy.length})</span>
            </div>
            <div className="bg-[#252526] rounded p-3">
  ...
  const renderBlobDetail = (blob: BlobObject) => {
    return (
      <div className="space-y-4">
        <div className="bg-yellow-500/5 border border-yellow-500/20 rounded p-4">
          ...
          <p className="text-xs text-gray-400 leading-relaxed">
            A blob stores the contents of a file. Git compresses and stores each version of each file as a separate blob. 
        ...
        {blob.referencedBy && blob.referencedBy.length > 0 && (
          <div>
            <div className="flex items-center gap-2 mb-2">
              <Link className="w-4 h-4 text-gray-400" />
              <span className="text-xs text-gray-400">Referenced By ({blob.referencedBy.length})</span>
            </div>
            <div className="bg-[#252526] rounded p-3">
